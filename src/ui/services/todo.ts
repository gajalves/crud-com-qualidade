...
async function getTodoWithPagination({page, limit} : GetTodoWithPaginationDto) : Promise<getTodoWithPaginationResponseDto> {
    return fetch(`api/todospagination?page=${page}&limit=${limit}`).then(async (response) => {
            if(response.ok) {
...
export const todoService = {
    get, getTodoWithPagination, create, toggleDone, deleteById
}
